...
export default function CatalogClient({ initialProducts }: { initialProducts: CatalogProduct[] }) {
  ...
  return (
    <div className="max-w-7xl mx-auto px-3 sm:px-4 lg:px-8 py-8 sm:py-12 space-y-6 sm:space-y-8">
      {/* Header Banner */}
      <div className="bg-[#FFF9F2] p-6 sm:p-8 md:p-12 rounded-2xl border border-[#E4D5C1] text-center space-y-4 shadow-2xs">
        ...
        <p className="text-sm md:text-base text-[#6E564F] max-w-2xl mx-auto leading-relaxed">
          Manzanas, uvas, charolas de toppings y paquetes preparados con chamoy. Pedidos con mínimo 24 horas de anticipación.
        </p>
      </div>

      {/* Category Filter Bar */}
      <div className="bg-white p-4 sm:p-6 rounded-2xl border border-[#E4D5C1] shadow-2xs space-y-4">
        ...
        <div className="flex flex-wrap gap-2 sm:gap-2.5 pt-1">
          {categories.map((cat) => (
            <button
              ...
              className={`px-3 sm:px-4 py-2 text-[10px] sm:text-xs font-bold uppercase tracking-wider rounded-full transition-all ${
      ...
      {filteredProducts.length > 0 ? (
        <div className="grid grid-cols-2 min-[520px]:grid-cols-3 sm:grid-cols-4 gap-3 sm:gap-4 lg:gap-6">
